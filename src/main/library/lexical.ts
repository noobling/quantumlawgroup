...
export function removeDoc(idx: LexicalIndex, docId: string): void {
  delete idx.docLen[docId]
  delete idx.snippets[docId]
  for (const term of Object.keys(idx.postings)) {
    const arr = idx.postings[term].filter(([id]) => id !== docId)
    if (arr.length) idx.postings[term] = arr
    else delete idx.postings[term]
  }
}
